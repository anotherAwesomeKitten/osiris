  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`fixed z-[400] ${fullscreen ? 'inset-4' : 'bottom-6 right-6 w-[420px]'}`}
      >
        <div className="glass-panel osiris-glow overflow-hidden h-full flex flex-col" style={{ borderColor: 'rgba(57, 255, 20, 0.3)' }}>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-secondary)]">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <h3 className="text-[11px] font-mono font-bold text-[#39FF14] tracking-wider truncate">{camera.name}</h3>
                <p className="text-[7px] font-mono text-[var(--text-muted)]">{camera.city}, {camera.country} · {camera.source}</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <button onClick={() => setFullscreen(!fullscreen)} className="p-1.5 rounded hover:bg-[var(--hover-accent)] transition-colors" title="Toggle fullscreen">
          ...
          <div className="px-4 py-2.5 border-t border-[var(--border-secondary)] flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {camera.feed_url && (
                <a href={camera.feed_url} target="_blank" rel="noopener noreferrer"
                  ...
                  <ExternalLink className="w-2.5 h-2.5" /> DIRECT FEED
                </a>
              )}
